  ...
  if (digitalRead(BTN_PIN) == HIGH) {
    ...
    while (true) {
      ...
      if (elapsedTime > lastTime) {
        lcd.setCursor(0, 1);
        lcd.print(READING_DURATION - elapsedTime);
        lcd.print(" ");
        
        if (elapsedTime < READING_DURATION ) {
          tone(BUZZ_PIN, 1000, 300);
        }
      }

      if (elapsedTime >= READING_DURATION) break;
      lastTime = elapsedTime;
    }

    float temp = dht.readTemperature();
    float humi = dht.readTemperature(true) - CALIBRATION;
